...
const TYPESCRIPT_FILES$1 = ["**/*/*.?([cm])ts", "**/*/*.?([cm])tsx"];
/**
...
function createConfig(overrides = {}) {
	...
	return {
		flat: {
			files: TYPESCRIPT_FILES$1,
			rules: rules$1
		},
		legacy: {
			overrides: [{ files: TYPESCRIPT_FILES$1 }],
			rules: rules$1
		}
	};
}
...
function isPossiblyType(type, callback) {
	return isTypeRecursive(type.getConstraint() ?? type, (innerType) => {
...
const RULE_NAME$23 = "lua-truthiness";
const FALSY_STRING_NUMBER_CHECK = "falsy-string-number-check";
const messages$23 = { [FALSY_STRING_NUMBER_CHECK]: "0, NaN, and \"\" are falsy in TS. If intentional, disable this rule by placing `\"roblox-ts/lua-truthiness\": \"off\"` in your eslint.config file in the \"rules\" object." };
...
//#endregion
//#region node_modules/.pnpm/ts-api-utils@2.1.0_typescript@5.9.3/node_modules/ts-api-utils/lib/index.js
...
const RULE_NAME$22 = "misleading-lua-tuple-checks";
...
function ensureArrayDestructuring(context, parserServices, leftNode) {
	if (isArrayBindingOrAssignmentPattern(parserServices.esTreeNodeToTSNodeMap.get(leftNode))) return;
...
function handleIterableFunction(context, parserServices, node, type) {
	...
	const aliasSymbol = parserServices.program.getTypeChecker().getTypeArguments(type)[0]?.aliasSymbol;
...
function create$21(context, [{ fixToUnknown }]) {
	return { TSAnyKeyword: (node) => {
		if (isNodeWithinKeyofAny(node)) return;
		const fixOrSuggest = {
...
const RULE_NAME$20 = "no-array-pairs";
...
function create$20(context) {
	...
	return { "CallExpression[callee.name=\"ipairs\"], CallExpression[callee.name=\"pairs\"]": (node) => {
		...
		if (!isArrayType(checker, getConstrainedTypeAtLocation(parserServices, node.arguments[0]))) return;
		context.report({
...
const RULE_NAME$19 = "no-enum-merging";
const ENUM_MERGING_VIOLATION = "enum-merging-violation";
const messages$19 = { [ENUM_MERGING_VIOLATION]: "Enum merging is not supported in roblox-ts. Declare all members in a single enum." };
...
const RULE_NAME$18 = "no-export-assignment-let";
const EXPORT_VIOLATION = "export-violation";
const messages$18 = { [EXPORT_VIOLATION]: "Cannot use `export =` on a `let` variable!" };
...
const RULE_NAME$17 = "no-for-in";
const FOR_IN_VIOLATION = "for-in-violation";
const messages$17 = { [FOR_IN_VIOLATION]: "For-in loops are forbidden because it always types the iterator variable as `string`. Use for-of or array.forEach instead." };
...
const RULE_NAME$16 = "no-function-expression-name";
const FUNCTION_EXPRESSION_VIOLATION = "function-expression-violation";
const messages$16 = { [FUNCTION_EXPRESSION_VIOLATION]: "Function expression names are not supported!" };
function create$16(context) {
	return { FunctionExpression(node) {
		const { id } = node;
		if (id === null) return;
		const referenced = context.sourceCode.getScope(node).set.get(id.name)?.references.some((ref) => ref.identifier !== id) ?? false;
		context.report({
...
const RULE_NAME$15 = "no-get-set";
const GET_SET_VIOLATION = "get-set-violation";
const messages$15 = { [GET_SET_VIOLATION]: "Getters and Setters are not supported for performance reasons. Please use a normal method instead." };
...
const RULE_NAME$14 = "no-implicit-self";
const COLON_VIOLATION = "violation";
const messages$14 = { [COLON_VIOLATION]: "Enforce the use of `.` instead of `:` for method calls" };
...
const RULE_NAME$13 = "no-invalid-identifier";
const { default: LuauAst } = createRequire(import.meta.url)("@roblox-ts/luau-ast");
...
const messages$13 = {
	...
	[RESERVED_IDENTIFIER]: "Avoid using '{{ identifier }}' as an identifier, as it is a reserved for usage by the roblox-ts compiler."
};
...
function getMessageId(name$1) {
	if (LuauAst.LUAU_RESERVED_KEYWORDS.has(name$1)) return INVALID_IDENTIFIER;
	if (RESERVED_KEYWORDS.has(name$1)) return RESERVED_IDENTIFIER;
...
function isRestricted(name$1) {
	return LuauAst.LUAU_RESERVED_KEYWORDS.has(name$1) || RESERVED_KEYWORDS.has(name$1) || !LuauAst.LUAU_IDENTIFIER_REGEX.test(name$1);
}
...
const RULE_NAME$12 = "no-namespace-merging";
const NAMESPACE_MERGING_VIOLATION = "namespace-merging-violation";
const messages$12 = { [NAMESPACE_MERGING_VIOLATION]: "Namespace merging is not supported in roblox-ts. Declare all members in a single namespace." };
function checkNamespaceMerging(context, node) {
	if (shouldSkipNode(node)) return;
	const variable = getNamespaceVariable(context, node);
	if (variable === void 0) return;
	if (!variable.defs.every((definition) => {
...
function create$12(context) {
	return { "TSModuleDeclaration[global!=true][id.type!='Literal']": function(node) {
...
function getNamespaceVariable(context, node) {
	...
	if (currentScope === null) return;
	const variable = currentScope.set.get(node.id.name);
	if (variable === void 0 || variable.defs.length <= 1) return;
	return variable;
}
...
const RULE_NAME$11 = "no-null";
const NULL_VIOLATION = "null-violation";
const messages$11 = { [NULL_VIOLATION]: "Usage of 'null' is not allowed. Use 'undefined' instead." };
...
const RULE_NAME$10 = "no-object-math";
...
function buildMethodCallFix({ fixer, left, macroName, operator, right, sourceCode }) {
	const { afterOp, beforeOp, hasParentheses } = extractOperatorContext(sourceCode.text.slice(left.range[1], right.range[0]), operator);
...
function createViolationContext({ context, macroName, node, operator, validation }) {
	const violationContext = {
		...
		node,
		operator,
		type: validation.isValid ? "math-operation" : "unsupported"
	};
...
function extractOperatorContext(textBetween, operator) {
	if (!(textBetween.includes(")") && textBetween.includes(operator))) return {
		afterOp: "",
		beforeOp: "",
	...
	return {
		afterOp: textBetween.slice(operatorIndex + operator.length).trimStart(),
		beforeOp,
		hasParentheses: true
	};
}
...
function getRobloxTypeFromBinaryExpr(node, parserServices) {
	const { left, operator, right } = node;
	if (mathOperationToMacroName.get(operator) === void 0) return;
	const validation = validateOperation({
		leftNode: left,
		leftType: getRobloxTypeName(left, parserServices),
		operator,
		rightNode: right,
		rightType: getRobloxTypeName(right, parserServices)
	});
...
function getSimpleRobloxType(node, parserServices) {
	return getRobloxDataTypeNameRecursive(getConstrainedTypeAtLocation(parserServices, node));
}
...
function processMathOperation({ context, leftDataType, node, operands, rightDataType }) {
	...
	reportViolation(createViolationContext({
		context,
		macroName,
		node,
		operator,
...
function tryOperandValidation({ macroName, operandType, otherNode, otherType }) {
	...
	}) ? createValidationResult(operandType, true, false) : void 0;
}
function trySwappedValidation({ macroName, operandType, otherNode, otherType }) {
	const constraints = getOperationConstraints(operandType, macroName);
	if (constraints?.allowSwapped !== true) return;
	return checkConstraints({
		constraints,
		otherNode,
		otherType,
		thisType: operandType
	}) ? createValidationResult(operandType, true, true) : void 0;
}
...
const RULE_NAME$9 = "no-post-fix-new";
const NEW_VIOLATION = "new-violation";
const messages$9 = { [NEW_VIOLATION]: "Calling .new() on objects without a .new() method is probably a mistake. Use `new X()` instead." };
...
function handleCallExpression(node, context, parserServices, checker) {
	...
	const hasNewProperty = checker.getTypeAtLocation(propertyAccess.expression).getProperty("new");
	if (!((hasNewProperty && isFunction(checker.getTypeOfSymbolAtLocation(hasNewProperty, propertyAccess.expression))) ?? false)) replaceWithNewExpression(context, node);
}
function replaceWithNewExpression(context, node) {
	context.report({
		fix: (fixer) => {
			...
			const replaced = !(accessNode.object.type === AST_NODE_TYPES.Identifier || accessNode.object.type === AST_NODE_TYPES.MemberExpression && !accessNode.object.computed) ? `new (${exprText})${argsText}` : `new ${exprText}${argsText}`;
...
const RULE_NAME$8 = "no-preceding-spread-element";
const PRECEDING_SPREAD_VIOLATION = "preceding-rest-violation";
const messages$8 = { [PRECEDING_SPREAD_VIOLATION]: "Spread element must come last in a list of arguments!" };
...
const RULE_NAME$7 = "no-private-identifier";
const PRIVATE_IDENTIFIER_VIOLATION = "private-identifier-violation";
const messages$7 = { [PRIVATE_IDENTIFIER_VIOLATION]: "Private identifiers (`#`) are not supported in roblox-ts. Use the 'private' access modifier instead." };
...
//#endregion
//#region src/rules/no-undeclared-scope/rule.ts
const RULE_NAME$6 = "no-undeclared-scope";
...
*/
function buildAllowedScopes(typeRoots, baseDirectory) {
	const allowedScopes = /* @__PURE__ */ new Map();
	if (typeRoots !== void 0 && typeRoots.length > 0) for (const root of typeRoots) {
...
*/
function checkModuleSource(allowedScopes, node, source, context) {
	if (!isScopedPackage(source)) return;
	...
	if (!existsSync(scopePath)) context.report({
		data: {
			path: scopePath,
			scope
		},
...
function create$6(context) {
	const { configFilePath, typeRoots } = getParserServices(context).program.getCompilerOptions();
	const allowedScopes = buildAllowedScopes(typeRoots, typeof configFilePath === "string" ? dirname(configFilePath) : context.getCwd());
	return {
		ExportAllDeclaration(node) {
			checkModuleSource(allowedScopes, node, node.source.value, context);
		},
		ExportNamedDeclaration(node) {
			if (node.source?.value !== void 0) checkModuleSource(allowedScopes, node, node.source.value, context);
		},
		ImportDeclaration(node) {
			checkModuleSource(allowedScopes, node, node.source.value, context);
		},
		ImportExpression(node) {
			if (node.source.type === AST_NODE_TYPES.Literal && typeof node.source.value === "string") checkModuleSource(allowedScopes, node, node.source.value, context);
		}
	};
}
/**
...
*/
function isScopedPackage(source) {
	return /^@[^/]+\//.test(source);
}
const noUndeclaredScope = createEslintRule({
	create: create$6,
	defaultOptions: [],
	meta: {
		docs: {
...
function create$4(context) {
	...
	return {
		...!allowTupleMacro && { "CallExpression[callee.type=\"Identifier\"][callee.name=\"$tuple\"]": function(node) {
...
function fixTupleMacroCall(node, context, fixer) {
	...
	const { sourceCode } = context;
	const replacementText = `[${args.map((argument) => sourceCode.getText(argument)).join(", ")}]`;
	return fixer.replaceText(node, replacementText);
}
function handleTypeReference(context, node) {
...
function hasDeclareProperty(node) {
	return node.type === TSESTree.AST_NODE_TYPES.VariableDeclaration || node.type === TSESTree.AST_NODE_TYPES.FunctionDeclaration || node.type === TSESTree.AST_NODE_TYPES.ClassDeclaration || node.type === TSESTree.AST_NODE_TYPES.TSModuleDeclaration;
}
function isInDeclareContext(node) {
	let current = node.parent;
	while (current) {
		if (DECLARE_CONTEXTS.has(current.type) || hasDeclareProperty(current) && current.declare) return true;
...
function isPlayersCallExpressionType(context, node, callee) {
	...
	if (!isBuiltinSymbolLike(parserServices.program, type, ["Players"])) return;
	if (!type.getProperty("GetPlayers")) return;
	check(context, node, callee);
}
...
* Can be used standalone or extended with other configurations.
*/
const legacyParserConfig = { overrides: [{
	files: TYPESCRIPT_FILES$1,
...
const configsWithoutNames = {
	...
	"eslintCompatLegacy": eslintCompatLegacy,
	"recommended": recommended,
	"recommended-legacy": recommendedLegacy,
	"ts-recommended-compat-legacy": tsRecommendedCompatLegacy,
	"tsRecommendedCompat": tsRecommendedCompat
};
function convertToKebabCase(str) {
	return str.replace(/[A-Z]+(?![a-z])|[A-Z]/g, (match, offset) => (offset ? "-" : "") + match.toLowerCase());
}
const configs = Object.fromEntries(Object.entries(configsWithoutNames).map(([key, config]) => {
	if (key.toLowerCase().includes("legacy")) return [key, { ...config }];
	return [key, {
		...config,
		name: `roblox-ts/${convertToKebabCase(key)}`
	}];
}));
...
//#endregion
export { TYPESCRIPT_FILES, configs, src_default as default, legacyParserConfig, rules };